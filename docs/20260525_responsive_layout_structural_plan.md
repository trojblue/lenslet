...
- [x] 2026-05-25: Sprint 3 started. Assumptions: native `inert` may isolate the browse shell only while viewer/compare overlays are active; similarity and move dialogs stay outside that inert shell; inspector fixes stay scoped to preview, rating row, action rows, and long value containment.
- [x] 2026-05-25: Sprint 3 implemented. `RSP-7` migrated viewer, compare, folder-resize, and upload drop overlay paths off the temporary `--left`/`--right` aliases; split inert handling so browse content is isolated under viewer/compare, compare also inerts the background toolbar, and viewer keeps its toolbar chrome usable. `RSP-8` added scoped inspector container sizing, responsive preview sizing, wrapping rating/action rows, and long-value containment.
- [x] 2026-05-25: Sprint 3 validation passed: focused inspector/viewer Vitest checks, `npx tsc --noEmit`, `python -m py_compile scripts/responsive_geometry_harness.py`, `npm run build`, packaged frontend regeneration, `python scripts/responsive_geometry_harness.py --output-json /tmp/lenslet-responsive-geometry-sprint3.json`, full `npm run test`, `python scripts/gui_smoke_acceptance.py --output-json /tmp/lenslet-gui-smoke-sprint3.json`, `python scripts/lint_repo.py`, `git diff --check`, and an `rg` check confirming no source or packaged frontend reads `--left`/`--right`.
- [x] 2026-05-25: Sprint 3 cleanup/review gates completed. Cleanup found only Tier 1 JSX formatting already clean locally. Initial review found viewer-toolbar inert coverage and generated-asset staging risks; both were addressed, the responsive harness was expanded to assert viewer toolbar usability and compare toolbar inertness, generated assets were staged, and follow-up review found no actionable issues.
- [x] 2026-05-25: Sprint 3 implementation handoff. Overlay left/right ownership now uses `--overlay-left`/`--overlay-right`, browse content is inert while viewer/compare is active, viewer toolbar controls remain live as viewer chrome, compare background toolbar is inert, and the inspector’s scoped internals stay inside the persistent panel when policy allows it.
- [x] 2026-05-25: Sprint 4 implemented. `RSP-9` finalized `scripts/responsive_geometry_harness.py` with a temporary Pillow/Parquet fixture dataset, richer DOM/debug evidence, health and server-log evidence, failure screenshots, deterministic visible-grid selection helpers, status/search/drawer/theme/overlay/inspector scenarios, resize persistence checks, and the `760px` metrics-left/both-sidebars-preferred-open selected-items observation.
- [x] 2026-05-25: Sprint 4 validation passed: focused metrics Vitest coverage, full frontend tests, frontend build with packaged bundle regeneration, responsive geometry harness with 18 passing scenarios, final zoom/page-scale responsive smoke for `320`, `360`, `390`, and short `1024x480` slices, GUI smoke acceptance, repo lint, `py_compile`, and `git diff --check`.
- [x] 2026-05-25: Sprint 4 cleanup/review gates completed. Cleanup applied only Tier 1 harness readability cleanup. Initial review found weak `R10` content-containment evidence; it was addressed by distinguishing intended resize-handle overflow from real left-panel content overflow, adding selected-metrics content assertions, and tightening the metrics panel/histogram layout. Follow-up review found no actionable issues.
- [x] 2026-05-25: Sprint 4 implementation handoff. The responsive geometry harness is now the final live-browser regression gate for `R1` through `R8`; `R9` and `R10` remain deferred unless they create primary acceptance failures, with `R10` selected-metrics containment now explicitly observed and asserted.
...
12. Sprint 3 migrated viewer/compare/drop overlay paths from the temporary `--left`/`--right` aliases to `--overlay-left`/`--overlay-right`; `FolderTree` resize handle now reads `--grid-left` directly, and the temporary aliases are no longer emitted.
13. Sprint 3 expanded `scripts/responsive_geometry_harness.py` with viewer/compare overlay scenarios, browse-shell inert evidence, viewer toolbar usability evidence, compare toolbar inert evidence, Escape close/focus-return checks, and scoped inspector containment checks.
14. Sprint 4 finalized `scripts/responsive_geometry_harness.py` with valid PNG/Parquet fixtures, browser/viewport/state evidence, DOM debug attributes, CSS variables, scroll dimensions, toolbar rectangles, sidebar/inspector/overlay rectangles, mobile drawer and theme settings reachability, health/server-log/failure evidence, the `760px` metrics-left selected-items observation, and content-overflow assertions that ignore only the intentional resize handle. The packaged frontend under `src/lenslet/frontend/` was regenerated after the metrics containment fixes.
