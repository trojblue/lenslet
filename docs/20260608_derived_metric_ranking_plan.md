...
1. [x] T2.1 Add formula-eligible numeric column selection beside, not inside, `is_metric_column_name`. Include existing metric-like numeric columns and short eval fields matching `q` followed by digits; exclude ids, row indices, source/path/name/mime, dimensions, size, timestamps, booleans, internal columns, and obvious counters/bookkeeping fields. Validation: unit tests cover included `q1`/`q2`/`q10` and excluded id/width/height/size/timestamp/boolean/path-like fields.
2. [x] T2.2 Use the helper in Parquet browse projection and table metric extraction without projecting arbitrary numeric schemas. Promoted formula-eligible numeric columns are owned by Metrics, so they should appear in item metrics and inspector Metrics, not duplicated under "Other table fields." Validation: table launch/index/display tests prove q-style columns are present as browse metrics and absent from table fields when their value is metric-owned.
3. [x] T2.3 Add storage-level `metric_keys()` support for table mode and make `src/lenslet/web/browse.py` prefer it when available, mirroring categorical key behavior. This is a hard acceptance criterion for Sprint 2 because null-only windows otherwise create false missing-input errors. Validation: route/storage tests prove schema-backed metric keys appear even if the loaded window has only null values for a formula input.
4. [x] T2.4 Verify the recent source-column switching path preserves projection and invalidation behavior. Validation: run `tests/web/routes/test_table_source_settings.py` plus a focused test where switching source columns keeps q-style `metric_keys` when the table schema still supports them.
...
- [x] 2026-06-08 10:39 UTC: Completed Sprint 2 tasks `T2.1` through `T2.4`. Added formula-eligible q-style metric column selection, projected q numeric inputs for Parquet browse, metric-owned q field suppression from table fields, storage-level `metric_keys()`, and projected source-switch candidate retention.
- [x] 2026-06-08 10:39 UTC: Ran Sprint 2 cleanup and review gates. Cleanup subagent `019ea6ba-4802-7ef3-b2dc-ab2e2605136d` applied one Tier 1 formatting cleanup; review subagent `019ea6bc-b757-7c91-abc1-dce55871becc` found three issues that were fixed; follow-up review `019ea6c9-1d8a-7630-85d4-430afb927e25` found one projection-performance issue that was fixed; final targeted review `019ea6cd-9c09-7bc0-81aa-43f0782678bc` found no actionable issues.
- [x] 2026-06-08 10:39 UTC: Validated Sprint 2 with `pytest tests/storage/table/test_table_formula_metrics.py tests/storage/table/test_table_index_pipeline.py tests/storage/table/test_parquet_ingestion.py tests/web/routes/test_table_source_settings.py tests/web/routes/test_direct_route_helpers.py tests/web/routes/test_folder_recursive.py -q`, focused `ruff`, `git diff --check`, and `python scripts/lint_repo.py`.
- [x] 2026-06-08 11:11 UTC: Completed Sprint 3 tasks `T3.1` through `T3.4`. Added the singular derived metric type/model, pure evaluator, AppDataScope wiring before filters/sort, metric display-name diagnostics, active derived key retention, and stale raw/derived metric reset guards.
- [x] 2026-06-08 11:11 UTC: Ran Sprint 3 cleanup and review gates. Cleanup subagent `019ea6dc-97c5-7441-bd4d-4ff9785112d8` found no Tier 1 cleanup to apply; review subagent `019ea6df-c6b8-76d0-acb0-15b4a27dee52` found three issues, all fixed; follow-up review subagent `019ea6e8-e383-7060-b1ed-09e745e66df4` found no actionable issues.
- [x] 2026-06-08 11:11 UTC: Validated Sprint 3 with `npm --prefix frontend test -- --run src/features/metrics/model/__tests__/derivedMetric.test.ts src/app/__tests__/appShellSelectors.test.ts`, `npm --prefix frontend test -- --run src/features/browse/model src/features/metrics src/app`, `npm --prefix frontend run build`, `git diff --check`, and `python scripts/lint_repo.py`. Build output under `frontend/dist/` was not copied to packaged assets.
- [x] 2026-06-08 11:13 UTC: Started Sprint 4 tasks `T4.1` through `T4.4`. Assumptions: local settings hard-cut only the view-state keys into one `viewState` JSON key while non-view UI preferences remain separate; malformed saved derived definitions are retained as raw JSON objects in `ViewState` and evaluated as invalid; unavailable derived metric-range filters remain in state and are surfaced through warnings instead of changing the filter engine.
- [x] 2026-06-08 11:33 UTC: Completed Sprint 4 tasks `T4.1` through `T4.4`. Added atomic local `viewState` persistence, Smart Folder view-state normalization and invalid-spec retention, unavailable derived sort/filter warning selectors, unavailable filter-chip labels, and source/scope revalidation coverage for saved specs.
- [x] 2026-06-08 11:33 UTC: Ran Sprint 4 cleanup and review gates. Cleanup subagent `019ea6fc-5c1b-78f2-b584-81a8926aa153` applied Tier 1 formatting/readability cleanup; review subagent `019ea700-624f-7df1-8b5f-8340adf58c49` found no actionable issues and noted residual end-to-end source-switch coverage remains for Sprint 5 browser acceptance.
- [x] 2026-06-08 11:33 UTC: Validated Sprint 4 with focused settings/Smart Folder/derived/status/filter-chip tests, `npm --prefix frontend test -- --run src/features/browse/model src/features/metrics src/app`, `npm --prefix frontend run build`, `pytest tests/web/routes/test_table_source_settings.py -q`, `git diff --check`, and `python scripts/lint_repo.py`. Build output under `frontend/dist/` was not copied to packaged assets.
- [x] 2026-06-08 12:23 UTC: Completed Sprint 5 tasks `T5.1` through `T5.4`. Added the Metrics-panel Derived Score card, metric display-name plumbing, Rank by score state transitions, derived-filter cleanup on Clear, and a table-backed GUI smoke workflow with q1-q6, `dataset_from`, Smart Folder restore, reload restore, and partial-load warning coverage.
- [x] 2026-06-08 12:23 UTC: Ran Sprint 5 cleanup and review gates. Cleanup subagent `019ea71f-3a85-7603-8b87-b110ecda8b44` applied a Tier 1 SVG title cleanup in `MetricScrollbar`; review subagent `019ea723-0820-7311-855e-6328184cfc48` found derived-filter cleanup and asset-staging issues; the filter cleanup was fixed; follow-up review subagent `019ea72b-e55f-7732-886b-94e940ab2d10` confirmed the source fix and found no additional source-level behavioral issues.
- [x] 2026-06-08 12:23 UTC: Validated Sprint 5 with `npm --prefix frontend test -- --run src/features/browse/model src/features/metrics src/app`, `pytest tests/storage/table/test_table_index_pipeline.py tests/storage/table/test_parquet_ingestion.py tests/web/routes/test_table_source_settings.py -q`, `pytest tests/scripts/test_direct_script_helpers.py -q`, `npm --prefix frontend run build`, `rsync -a --delete frontend/dist/ src/lenslet/frontend/`, `python -m scripts.browser.gui_smoke.acceptance`, `git diff --check`, and `python scripts/lint_repo.py`. Packaged frontend assets under `src/lenslet/frontend/` were regenerated.
...
Sprint 2 handoff:
        ...
        Added formula-eligible q-style metric selection beside `is_metric_column_name`, with tests for q inclusions and id/path/dimension/timestamp/boolean/bookkeeping exclusions.
        Updated Parquet projection and table extraction so numeric `q1`/`q2`/`q10` fields are owned by Metrics, while string q fields remain visible table fields.
        Added `TableStorage.metric_keys()` and browse route support so schema-backed q metrics are advertised even when the current loaded folder/window has only null values.
        Kept metrics-map keys scoped to loadable rows, and retained projected source-switch candidates by sampling only readable scalar string candidate columns.
        Packaged frontend assets were not regenerated in Sprint 2 because no frontend bundle files changed.

Sprint 3 handoff:

        Added `frontend/src/features/metrics/model/derivedMetric.ts` with v1 spec normalization, stable `@derived/<id>` keys, input-key extraction, pure score evaluation, valid/unavailable/invalid diagnostics, reserved raw `@derived/*` metric stripping, and clone-only-when-needed behavior.
        Extended `ViewState` with a singular optional `derivedMetric` plus `MetricDisplayNames`.
        Wired derived evaluation through `useAppDataScope` after star overrides and source key resolution, before filtering/sorting and metric UI data. Search/similarity scopes use loaded result counts for derived partial-load metadata, while normal folder scopes keep folder total warnings.
        Updated AppShell metric selection/sort validation so missing raw metric sorts still reset, stale derived sorts reset, and active valid/unavailable/invalid derived sorts are retained by stable key.
        Added focused frontend tests for spec normalization, invalid malformed definitions, weighted numeric terms, categorical bonuses, missing-key diagnostics, all-invalid scores, no mutation, similarity scan limits, retained derived sorts, and search/similarity partial metadata.
        Packaged frontend assets were not regenerated in Sprint 3. `npm --prefix frontend run build` was used as a validation check only.

Sprint 4 handoff:

        Hard-cut app shell local storage to a single `viewState` JSON key for filters, sort, selectedMetric, and derivedMetric while keeping non-view UI preferences separate. The write path removes legacy `sortSpec`, `filterAst`, and `selectedMetric` keys; the read path does not migrate them.
        Widened stored `ViewState.derivedMetric` so malformed saved definitions remain as raw JSON objects. `evaluateDerivedMetric` still decides validity at runtime and surfaces invalid diagnostics without scoring invalid specs.
        Normalized Smart Folder save, activation, export, and stale-view comparison through `normalizeViewState`, preserving invalid derived definitions and detecting derived spec changes.
        Added AppShell/status/filter-chip warnings for active derived sorts and metricRange filters when the derived key is stale, invalid, missing inputs, has no valid values, or ranks only a loaded window.
        Added tests for atomic settings restore, Smart Folder roundtrip/staleness, invalid spec retention, unavailable derived warnings, unavailable derived filter chips, source/scope revalidation of saved specs, and table source-switch route behavior.
        Packaged frontend assets were not regenerated in Sprint 4. `npm --prefix frontend run build` was used as a validation check only.

Sprint 5 handoff:

        Added `frontend/src/features/metrics/components/DerivedScoreCard.tsx`, `frontend/src/features/metrics/model/derivedMetricDraft.ts`, and `frontend/src/lib/metricDisplay.ts` for the one-score builder, draft/spec conversion, formula preview, rank gating, and display-name lookup.
        Threaded `metricDisplayNames` through toolbar sort options, Metrics panel selectors/cards, filter chips, inspector metrics, and the metric rail so primary UI labels show the derived score name instead of raw `@derived/...` ids.
        Added AppShell view-state helpers so Rank by score applies the draft, sorts descending by the stable derived key, and selects that metric; clearing the score also removes derived metric-range filters while preserving unrelated filters.
        Extended GUI smoke acceptance with a generated table-backed Parquet fixture exposing `q1` through `q6` plus `dataset_from`; the browser path creates `new_score`, ranks by it, verifies `/ranked/item_0001.jpg` first, saves/restores a Smart Folder, reloads, and observes the 5000-of-5006 loaded-window warning.
        Regenerated packaged frontend assets under `src/lenslet/frontend/` with `npm --prefix frontend run build` and `rsync -a --delete frontend/dist/ src/lenslet/frontend/`.

All planned sprints are complete. Residual deferred work remains as scoped in this plan: backend global derived sorting, multiple score presets, richer categorical weighting, export, calibration, and rubric comparison views.
