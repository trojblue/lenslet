...
- [x] 2026-06-09: Sprint 2 implementation completed. Added native `TableStorage.query_browse_scope` over row ids, row-level query records for metrics/categoricals/dimensions/source/url/star/notes/search text, final-window-only item materialization, and focused storage/web tests for filtered totals, sidecar filters, source-search toggle parity, and separated raw facets.
- [x] 2026-06-09: Sprint 2 cleanup pass completed with the `code-simplifier` routine. Optional non-semantic cleanups were applied for long row-field unpack formatting, reusing computed folder entries, and isolating materialization-count assertions between sidecar subcases.
- [x] 2026-06-09: Sprint 2 review pass completed with the `code-review` routine. Fix applied: native table browse-query text search now respects `include_source_in_search` for row source/url fields while still allowing explicit sidecar source/url text. Follow-up review found no remaining Sprint 2 issues.
...
Sprint 1 handoff, 2026-06-09: backend query contract and evaluator are in place. New/changed files are `src/lenslet/browse/query.py`, `src/lenslet/browse/__init__.py`, `src/lenslet/web/models.py`, `src/lenslet/web/routes/folders.py`, `src/lenslet/web/browse.py`, `src/lenslet/storage/base.py`, `tests/browse/test_query_evaluator.py`, and `tests/web/routes/test_browse_query.py`. Validation evidence: `pytest -q tests/browse/test_query_evaluator.py tests/web/routes/test_browse_query.py`, `pytest -q tests/web/routes/test_direct_route_helpers.py tests/web/routes/test_folder_recursive.py tests/api/test_import_contract.py`, `pytest -q tests/storage/table/test_parquet_ingestion.py`, and `python scripts/lint_repo.py` passed.

Sprint 2 handoff, 2026-06-09: native table row-query execution and backend facet separation are in place. New/changed files are `src/lenslet/storage/table/storage.py`, `tests/storage/table/test_table_browse_query_storage.py`, `tests/web/routes/test_browse_query.py`, `docs/20260609_backend_browse_query_plan.md`, and `docs/ralph/20260609_backend_browse_query_filtering/progress.txt`. Validation evidence: `pytest -q tests/storage/table/test_table_browse_query_storage.py tests/web/routes/test_browse_query.py`, `pytest -q tests/storage/table/test_table_browse_query_storage.py tests/storage/table/test_parquet_ingestion.py tests/storage/table/test_table_index_pipeline.py tests/storage/search/test_search_text_contract.py tests/web/routes/test_browse_query.py`, `pytest -q tests/web/routes/test_direct_route_helpers.py tests/web/routes/test_folder_recursive.py tests/api/test_import_contract.py`, and `python scripts/lint_repo.py` passed. Cleanup and review subagents completed; the review-found source-search toggle parity issue was fixed and follow-up review found no remaining Sprint 2 issues.

Sprint 3 handoff, 2026-06-09: frontend hard cutover to backend browse query windows is in place. New/changed files are `frontend/src/lib/types.ts`, `frontend/src/lib/fetcher.ts`, `frontend/src/api/client.ts`, `frontend/src/api/folders.ts`, `frontend/src/api/items.ts`, `frontend/src/app/AppShell.tsx`, `frontend/src/app/hooks/useAppDataScope.ts`, `frontend/src/app/hooks/useAppSyncEvents.ts`, `frontend/src/app/hooks/useFolderRefreshActions.ts`, `frontend/src/app/model/appShellSelectors.ts`, `frontend/src/app/model/appShellStateSync.ts`, `frontend/src/features/metrics/components/CategoricalPanel.tsx`, `frontend/src/features/metrics/components/MetricRangePanel.tsx`, `frontend/src/api/__tests__/folders.test.ts`, `frontend/src/app/hooks/__tests__/useAppDataScopeBackendQuery.test.ts`, `frontend/src/app/__tests__/appShellSelectors.test.ts`, `frontend/src/app/__tests__/liveUpdateCachePatch.test.ts`, and `frontend/src/features/metrics/__tests__/MetricsPanel.test.tsx`. Validation evidence: `cd frontend && npm test -- --run src/api src/app/hooks src/features/metrics`, `cd frontend && npm run build`, `python scripts/lint_repo.py`, and `git diff --check` passed. Cleanup and review subagents completed; review-found queued abort, search/facet, sidecar membership, and scan-stable derived-sort issues were fixed and follow-up review found no remaining Sprint 3 issues.

Sprint 4 handoff, 2026-06-09: real scenario validation and packaged frontend are complete. New/changed files are `scripts/browser/gui_smoke/acceptance.py`, `scripts/browser/gui_smoke/fixtures.py`, `scripts/browser/gui_smoke/scenarios.py`, `src/lenslet/frontend/`, `docs/20260609_backend_browse_query_plan.md`, `docs/ralph/20260609_backend_browse_query_filtering/progress.txt`, `docs/ralph/20260609_backend_browse_query_filtering/iteration4_gui_smoke_acceptance.json`, and `docs/ralph/20260609_backend_browse_query_filtering/iteration4_real_parquet_validation.json`. Validation evidence: `npm test -- --run` in `frontend/` passed (469 tests), `npm run build` in `frontend/` passed, `rsync -a --delete frontend/dist/ src/lenslet/frontend/` regenerated packaged assets, `python scripts/lint_repo.py` passed, `git diff --check` passed, `python -m scripts.browser.gui_smoke.acceptance --output-json docs/ralph/20260609_backend_browse_query_filtering/iteration4_gui_smoke_acceptance.json` passed, and the real parquet browser check saved `iteration4_real_parquet_validation.json` with `filtered_total = 3,264`. Playwright Chromium dependencies were installed locally with `python -m playwright install --with-deps chromium` after the first browser launch failed on missing `libnspr4.so`.
