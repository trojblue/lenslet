...
- [x] V1-T1: Run the reported large Parquet workflow and record source/status, dimension immutability, backend-filter truth, metric sort payload bounds, and media failure behavior.
- [x] V1-T2: Run browse/grid browser acceptance for filter, search, sort, viewer next/prev, folder scope change, and empty/error states.
- [x] V1-T3: Run metrics/derived query identity tests and a large table metric sort scenario.
- [x] V1-T4: Build, package, lint, and record evidence.
...
- [x] 2026-06-10: Phase 1 Validation Sprint completed, V1-T1 through V1-T4 landed. Large Parquet validation used a generated 50,006-row table and passed source immutability, launch status, workspace dimension cache, backend-filter, bounded metric-sort, oversized-window rejection, and media-failure checks. Browser acceptance passed with the existing non-strict folder re-entry anchor warning; primary large-tree smoke passed with first grid 3.71s, hotpath 3068ms, first thumbnail 3578ms, max frame gap 433.2ms, request peaks `folders=2`, `thumb=6`, `file=0`, and four console missing-resource warnings. Validation closed residual full-suite debt: q-style missing/numeric-string metric keys are preserved without promoting generic numeric strings, full pytest passes, full Vitest passes, `npx tsc --noEmit` passes, frontend build and package build pass, and `python scripts/lint_repo.py` passes with warn-only file-size notices. Cleanup gate applied one Tier 1 test helper simplification; review gate found one metric-detection broadening issue, which was fixed and confirmed resolved.
...
Phase 1 validation handoff: Phase 1 is complete. Implementation cleanup artifacts include q-style missing/numeric-string metric-key preservation in `src/lenslet/storage/table/index.py`, updated validation helper tests, and the URL typecheck fix in `frontend/src/app/routing/__tests__/viewStateUrl.test.ts`. Evidence artifacts were written under ignored `data/fixtures/`: `phase1_large_parquet_validation.json`, `gui_smoke_phase1_validation.json`, and `large_tree_40k_smoke_result.json`. Final validation passed: live large-Parquet probe, `python -m scripts.browser.gui_smoke.acceptance --output-json data/fixtures/gui_smoke_phase1_validation.json`, `python -m scripts.browser.large_tree.smoke --dataset-dir data/fixtures/large_tree_40k --output-json data/fixtures/large_tree_40k_smoke_result.json`, focused backend/frontend contract suites, `pytest -q`, `cd frontend && npm test -- --run`, `cd frontend && npx tsc --noEmit`, `cd frontend && npm run build`, `rsync -a --delete frontend/dist/ src/lenslet/frontend/`, `python -m build`, and `python scripts/lint_repo.py`. Residual warnings are non-blocking: GUI smoke still records the non-strict folder re-entry exact-anchor warning, large-tree smoke recorded four console missing-resource warnings while passing all thresholds, Vite reports the existing large chunk warning, and lint reports warn-only file-size guardrails for four large files.
