...
Sprint 2 is open after the iteration-4 engine slice. S2-T1 through S2-T6 now provide immutable compact columns, selective mutable dependency generations, cancellation-aware filtering, derived-score analysis, and stable ordering with generic-evaluator parity. Production browse and facet routes intentionally remain on the generic evaluator until S2-T7 slices before item construction and closes the hard cutover. The controlled 2,000-row probe remains below one second at 309.582 milliseconds combined p95; this is a no-regression checkpoint, not the final columnar-route performance result.
...
Sprint 2 iteration-4 handoff: S2-T1 through S2-T6 are complete in `src/lenslet/storage/table/query_engine.py`; `TableStorage.query_engine` owns the launch-built engine and mirrors sidecar updates into it. The next iteration must execute S2-T7 only before the sprint-boundary gates: cut `query_browse_scope` and query facets to the engine, slice ordered row IDs before materializing items, retain the generic path only as a test oracle, and then run the required code-simplifier and independent review routines before marking Sprint 2 complete. Do not begin Sprint 3 in that iteration.
