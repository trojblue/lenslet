...
At plan kickoff, table mode still constructed dense objects. `src/lenslet/storage/table/storage.py` defined `TableBrowseItem`, called `build_table_indexes(... item_factory=TableBrowseItem ...)`, bound `result.items`, built `_sorted_paths` and `_sorted_items`, and searched over `_sorted_items`. `src/lenslet/storage/table/row_scan.py` had the uniform HTTP fast path, but it still built `items`, `path_to_row`, `row_to_path`, `row_dimensions`, folder indexes, and item objects. `src/lenslet/storage/index_assembly.py` was the generic dense assembly path and should not be broadly rewritten until table startup no longer depends on it.
...
Sprint 4 final result, recorded in `docs/ralph/20260603_table_row_view_backend/iteration4_final_harness.json`: table-specific dense startup code is removed (`TableBrowseItem`, `build_table_indexes`, dense `scan_rows`, and dense uniform HTTP index assembly), while generic `index_assembly.py` remains for dataset/local storage. The measured row-store trim omits `row_to_slot` for identity/no-skip tables; the target profile reports it as `NoneType` with a 16-byte container instead of the previous 500k-entry list. The 500k harness reported `prepare_table_launch` 3.339s, RSS 827.4 MiB after launch and 856.9 MiB peak after checks, recursive 5k window 0.949s, repeated search miss median 0.168s, 0 dense items, 0 sorted items, 0 retained folder item refs, and 5k materialized items after the recursive window. Primary launch/RSS targets remain unmet, but the remaining bottleneck is explicitly the retained Arrow/NumPy metric columns, Python path/source/name row arrays, `path_to_row`/`row_to_path`, folder row refs, row dimension overlays, and lowercased path/name search caches after the first search, not a dense browse-item graph.
...
- [x] 2026-06-03: S4-T1 removed proven-unused table-specific dense startup code: `TableBrowseItem`, its package export, `build_table_indexes`, dense `scan_rows`, dense uniform HTTP assembly, and the old row-scan unit tests. Generic dataset/local `index_assembly.py` remains intact.
- [x] 2026-06-03: S4-T2 applied the measured retained-cache trim by making `TableRowStore.row_to_slot` identity-lazy. Dense no-skip tables now retain no slot map, while skipped-row tables still allocate and backfill the explicit map. The 500k profile confirms `row_to_slot` is `NoneType`.
- [x] 2026-06-03: S4-T2 final harness recorded `prepare_table_launch` 3.339s, RSS 827.4 MiB after launch and 856.9 MiB peak after checks, recursive 5k window 0.949s, repeated search miss median 0.168s, 0 dense items, 0 sorted items, 0 retained folder item refs, and 5k materialized items after the recursive window.
- [x] 2026-06-03: S4-T3 validation passed: focused table/search/dataset suite, small harness parity compare, full 500k harness, `PYTHONPATH=src python scripts/lint_repo.py`, `PYTHONPATH=src python -m pytest -q`, browser acceptance smoke, and a live 500k server browse/search probe. Live server results: health ready for 500k rows, recursive 5k window 0.387s over HTTP, and warm search miss median 0.129s with zero hits. Cleanup subagent made Tier 1 import/docstring wording edits only; review subagent reported no actionable findings.
- [x] Sprint 4 complete: proven-unused dense table startup deletion, measured retained-cache trim, and real-scenario validation.
...
- `docs/ralph/20260603_table_row_view_backend/iteration4_final_harness.json` for Sprint 4 dense-code deletion, slot-map trim, final 500k benchmark, and retained-category profiling.
...
Sprint 4 final handoff note:

The planned row-view backend is complete. Table mode no longer has a table-specific dense startup item path, the old dense table scan/build entrypoints are gone, and the target 500k workflow was validated through the harness, browser smoke, and a live server browse/search probe. The aspirational `prepare_table_launch <= 2.0s` and RSS `<750MB` targets were not reached, but the remaining retained categories are measured and no longer include `TableBrowseItem` ownership or folder item lists. Further reductions would require a new approved scope around dropping duplicated path/name/source arrays, narrowing row lookup structures, or moving toward true lazy/windowed parquet reads.
