...
2. **Narrow host checks.** The regex only matches exactly `localhost`, `127.0.0.1`, or `[::1]`. Cases like `foo.localhost`, `0.0.0.0`, or custom dev hostnames (`lenslet.local`) aren’t treated as local. A small change to environment or how you access the site can flip the branch and send requests to the wrong origin/port.
...
fetch(`${BASE}/health`).catch(() => {
  if (import.meta.env?.DEV) {
    console.warn(`[lenslet] API not reachable at ${BASE || 'same-origin'}. Falling back to same-origin.`)
  }
})
```
