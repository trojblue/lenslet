...
PLAN_FILE_DEFAULT="$REPO_ROOT/docs/20260211_post_s4_high_impact_simplification_scan.md"
...
if [ ! -f "$PROGRESS_FILE" ]; then
  {
    ...
    echo "- Capture simplification opportunities with concrete file/test anchors."
    echo "- Preserve behavior unless a plan task explicitly approves a contract change."
    echo ""
  } > "$PROGRESS_FILE"
fi
