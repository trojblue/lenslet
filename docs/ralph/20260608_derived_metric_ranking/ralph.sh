...
Config file (optional): ralph.config.json in this directory. Supports sharedDir for reusable prompt assets.
USAGE
}
...
if [[ -f "$CONFIG_FILE" ]]; then
  ...
  CFG_SHARED_DIR="$(jq -r '.sharedDir // empty' "$CONFIG_FILE")"
  ...
  [[ -n "$CFG_SHARED_DIR" ]] && SHARED_DIR_RAW="$CFG_SHARED_DIR"
fi
...
if [[ -n "$SHARED_DIR_RAW" ]]; then
  DEFAULT_CODEX_PROMPT="$(resolve_path "$SHARED_DIR_RAW")/prompt.md"
  DEFAULT_CLAUDE_PROMPT="$(resolve_path "$SHARED_DIR_RAW")/CLAUDE.md"
fi
if [[ "$MODE" == "plan" ]]; then
  DEFAULT_CODEX_PROMPT="$SCRIPT_DIR/prompt-plan.md"
  DEFAULT_CLAUDE_PROMPT="$SCRIPT_DIR/CLAUDE-plan.md"
  if [[ -n "$SHARED_DIR_RAW" ]]; then
    DEFAULT_CODEX_PROMPT="$(resolve_path "$SHARED_DIR_RAW")/prompt-plan.md"
    DEFAULT_CLAUDE_PROMPT="$(resolve_path "$SHARED_DIR_RAW")/CLAUDE-plan.md"
  fi
fi
