...
function QuickViewSectionComponent({
  ...
  return (
    ...
    >
      ...
      <div className="space-y-1.5 text-[12px]">
        {rows.map((row) => (
          <div key={row.id} className="ui-kv-row items-start gap-2">
            ...
            <span className="ui-kv-value flex-1 break-words whitespace-pre-wrap text-left">
              {row.value || '—'}
            </span>
            <button
              type="button"
      ...
      <div className="space-y-1.5 rounded-md border border-border/60 bg-surface-inset/40 p-2">
        <div className="text-[10px] uppercase tracking-wide text-muted">Custom JSON paths</div>
        <textarea
