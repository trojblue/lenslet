...
export default function DerivedMetricMiniHistogram({
  ...
  if (!histogram) {
    return (
      <div
        className="rounded-sm border border-border/50 bg-surface/60 px-2 py-1 text-[10px] text-muted"
  ...
  return (
    <div className="space-y-0.5" data-derived-metric-histogram={metricKey}>
      <svg
        viewBox={`0 0 ${width} 32`}
        preserveAspectRatio="none"
        className="h-9 w-full rounded-sm border border-border/50 bg-surface/60"
