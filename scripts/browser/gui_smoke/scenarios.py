...
def folder_query_request_count(page: Page) -> int:
    raw = page.evaluate(
        """() => performance.getEntriesByType('resource')
          .filter((entry) => typeof entry.name === 'string' && entry.name.includes('/folders/query'))
          .length"""
    )
    if not isinstance(raw, int):
        raise SmokeFailure("Failed to inspect browser resource timing entries.")
    return raw
...
def run_derived_metric_workflow(page: Page, timeout_ms: float) -> DerivedMetricSmokeResult:
    ...
    query_requests_before_rank = folder_query_request_count(page)
    rank_button.click()
    ...
    query_requests_after_rank = folder_query_request_count(page)
    if query_requests_after_rank != query_requests_before_rank:
        raise SmokeFailure(
            "Derived metric backend-browse sort sent a folder-query request instead of failing closed: "
            f"before={query_requests_before_rank}, after={query_requests_after_rank}."
        )
