...
ImageMime = Literal["image/webp", "image/jpeg", "image/png"]
...
def normalize_image_mime(value: object, fallback_name: str) -> ImageMime:
    normalized = str(value or "").strip().lower()
    if normalized in {"image/webp", "webp"}:
        return "image/webp"
    if normalized in {"image/png", "png"}:
        return "image/png"
    if normalized in {"image/jpeg", "image/jpg", "jpeg", "jpg"}:
        return "image/jpeg"
    return guess_image_mime(fallback_name)
...
def make_webp_thumbnail(
    ...
    *,
    ...
    with Image.open(BytesIO(img_bytes)) as image:
        ...
        if short_side > thumb_size:
            scale = thumb_size / short_side
