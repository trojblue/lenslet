...
from typing import Any, BinaryIO, Dict, List, TypeAlias

from PIL import Image, PngImagePlugin, ExifTags


PNG_SIGNATURE = b"\x89PNG\r\n\x1a\n"
ImageMetadataInput: TypeAlias = str | PathLike[str] | BinaryIO
...
def _read_bytes(file_obj: ImageMetadataInput) -> bytes:
    if hasattr(file_obj, "read"):
        data = file_obj.read()
        if not isinstance(data, bytes):
            raise TypeError("metadata reader file-like input must return bytes")
        return data
...
def _decode_bytes(value: bytes) -> str:
    for enc in ("utf-8", "latin-1"):
        decoded = _decode_bytes_with(value, enc)
        if decoded is not None:
            return decoded
    return repr(value)


def _decode_bytes_with(value: bytes, encoding: str) -> str | None:
    try:
    ...
    if len(value) >= 8:
        ...
        if prefix.startswith(b"UNICODE"):
            for enc in ("utf-16-le", "utf-16-be", "utf-8"):
                decoded = _decode_bytes_with_errors(body, enc, "replace")
                if decoded is not None:
...
def _decode_bytes_with_errors(value: bytes, encoding: str, errors: str) -> str | None:
    try:
...
def _parse_png_text_chunks(data: bytes) -> List[Dict[str, Any]]:
    ...
    while pos + 8 <= len(data):
        ...
        if ctype == b"tEXt":
            chunk = _parse_png_text_chunk(cdata)
            if chunk is not None:
                out.append(chunk)

        elif ctype == b"zTXt":
            chunk = _parse_png_ztxt_chunk(cdata)
            if chunk is not None:
                out.append(chunk)

        elif ctype == b"iTXt":
            chunk = _parse_png_itxt_chunk(cdata)
            if chunk is not None:
                out.append(chunk)
...
def _parse_png_text_chunk(cdata: bytes) -> dict[str, str] | None:
    # Latin-1: key\0value
    try:
        ...
        return None
    return {"type": "tEXt", "keyword": key, "text": text}


def _parse_png_ztxt_chunk(cdata: bytes) -> dict[str, str] | None:
    ...
    try:
        ...
        return None
    return {"type": "zTXt", "keyword": key, "text": text}


def _parse_png_itxt_chunk(cdata: bytes) -> dict[str, str] | None:
...
def read_png_info(file_obj: ImageMetadataInput) -> dict[str, Any]:
    """
...
def read_jpeg_info(file_obj: ImageMetadataInput) -> dict[str, Any]:
    data = _read_bytes(file_obj)
...
def read_webp_info(file_obj: ImageMetadataInput) -> dict[str, Any]:
    data = _read_bytes(file_obj)
