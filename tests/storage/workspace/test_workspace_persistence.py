...
from lenslet.web.app.factory import create_app
from lenslet.web.sync.labels import _load_label_state
...
def test_temp_workspace_root_resolves_gettempdir_lazily(
    monkeypatch,
    tmp_path: Path,
) -> None:
    ...
    workspace = Workspace.for_temp_dataset(dataset_root)

    assert workspace.root is not None
    assert workspace.root.parent == temp_root / "lenslet"
    assert workspace.is_temp_workspace() is True
...
def test_load_label_state_fails_on_corrupt_workspace_persistence(tmp_path: Path) -> None:
    ...
    class _Storage:
        def ensure_sidecar(self, _path: str) -> dict:
            return {"version": 1}

        def set_sidecar(self, _path: str, _meta: dict) -> None:
            return None
...
def test_views_route_fails_when_workspace_views_are_corrupt(tmp_path: Path) -> None:
    ...
    assert response.json() == {
        "error": "views_unreadable",
        "message": "workspace views are unreadable",
    }


def test_views_routes_publish_error_response_contracts(tmp_path: Path) -> None:
    root = tmp_path / "gallery"
    root.mkdir()
    Image.new("RGB", (4, 4), color=(32, 64, 128)).save(root / "sample.jpg", format="JPEG")

    schema = create_app(str(root)).openapi()
    get_responses = schema["paths"]["/views"]["get"]["responses"]
    put_responses = schema["paths"]["/views"]["put"]["responses"]

    assert get_responses["500"]["content"]["application/json"]["schema"]["$ref"].endswith("/ErrorResponse")
