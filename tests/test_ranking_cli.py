...
def test_cli_rank_token_can_still_target_local_rank_directory(monkeypatch, tmp_path: Path) -> None:
    ...
    assert captured["root_path"] == str(rank_dir.resolve())
    assert captured["uvicorn_app"] is sentinel_app


def test_cli_rank_token_with_browse_flags_still_targets_local_rank_directory(
    monkeypatch,
    tmp_path: Path,
) -> None:
    rank_dir = tmp_path / "rank"
    ...
    cli.main(["rank", "--port", "7090"])

    assert captured["root_path"] == str(rank_dir.resolve())
    assert captured["uvicorn_app"] is sentinel_app
    assert captured["port"] == 7090
