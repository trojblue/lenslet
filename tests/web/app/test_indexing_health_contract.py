    ...
    **kwargs,
    ...
    def _disable_preindex(
        ...
        *,
        ...
        _ = thumb_size
        _ = thumb_quality
        _ = skip_dimension_probe
        return None, workspace, preindex_signature

    monkeypatch.setattr(local_app, "ensure_preindex_storage", _disable_preindex)
    return create_app(
        str(root_path),
        options=LocalAppOptions(browse=BrowseAppOptions(indexing_listener=indexing_listener)),
        **kwargs,
    )
...
def test_memory_health_reports_running_then_ready_indexing_state(
    ...
    original_list_dir = LocalStorage.list_dir
    started = threading.Event()
    release = threading.Event()

    def _blocked_list_dir(self: LocalStorage, path: str):
        started.set()
        if not release.wait(timeout=2.0):
            raise RuntimeError("test timeout waiting to release index build")
        return original_list_dir(self, path)

    monkeypatch.setattr(LocalStorage, "list_dir", _blocked_list_dir)
...
def test_memory_health_reports_error_when_warm_index_fails(
    ...
    def _failing_list_dir(self: LocalStorage, path: str):
        _ = self
        _ = path
        raise RuntimeError("forced warm index failure")

    monkeypatch.setattr(LocalStorage, "list_dir", _failing_list_dir)
...
def test_memory_health_reports_error_when_item_indexing_fails(
    ...
    original_resolve_path = LocalStorage.resolve_path

    def _failing_resolve_path(self: LocalStorage, path: str) -> str:
        if path.endswith("/broken.jpg"):
            raise OSError("forced item failure")
        return original_resolve_path(self, path)

    monkeypatch.setattr(LocalStorage, "resolve_path", _failing_resolve_path)
    ...
    [
        ...
        pytest.param(
            lambda root: create_app_from_table(
                ...
                options=TableAppOptions(base_dir=str(root / "table")),
            ),
            id="table",
        ),
        pytest.param(
...
def test_table_workspace_id_differs_for_distinct_tables_with_same_base_dir(tmp_path: Path) -> None:
    ...
    app_a = create_app_from_table(
        ...
        options=TableAppOptions(base_dir=str(base_dir)),
    )
