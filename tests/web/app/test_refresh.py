...
from lenslet.server import LocalAppOptions, create_app, create_app_from_datasets
...
def _trusted_app(root: Path, **options: object):
    return create_app(
        str(root),
        options=LocalAppOptions(trusted_write_origins=(LOCAL_ORIGIN,), **options),
    )
...
def test_invalidate_subtree_drops_cache_and_rebuilds(tmp_path: Path):
    ...
    # Prime caches
    idx_a = storage.load_index("/a")
    idx_b = storage.load_index("/b")
    assert len(idx_a.items) == 1
    assert len(idx_b.items) == 1

    storage.get_or_build_thumbnail("/a/one.jpg")
    storage.load_dimensions("/a/one.jpg")
    sidecar_state = storage.ensure_sidecar("/a/one.jpg")
    sidecar_state["notes"] = "stale"
    storage.set_sidecar("/a/one.jpg", sidecar_state)
    assert storage.get_cached_thumbnail("/a/one.jpg") is not None
    assert "/a/one.jpg" in dict(storage.sidecar_items())
    ...
    assert len(storage.load_index("/a").items) == 1

    storage.invalidate_subtree("/a")

    refreshed = storage.load_index("/a")
    ...
    assert "/a/one.jpg" not in dict(storage.sidecar_items())
    assert len(storage.load_index("/b").items) == 1


def test_invalidate_subtree_can_preserve_sidecar_state(tmp_path: Path):
    root = tmp_path
    folder = root / "a"
    img = folder / "one.jpg"
    _make_image(img)
    ...
    storage = MemoryStorage(str(root))
    storage.load_index("/a")
    storage.get_or_build_thumbnail("/a/one.jpg")
    storage.load_dimensions("/a/one.jpg")

    sidecar_state = storage.ensure_sidecar("/a/one.jpg")
    sidecar_state["notes"] = "keep me"
    sidecar_state["tags"] = ["tagged"]
    sidecar_state["star"] = 4
    storage.set_sidecar("/a/one.jpg", sidecar_state)

    storage.invalidate_subtree("/a", clear_sidecars=False)

    assert storage.get_cached_thumbnail("/a/one.jpg") is None

    preserved = storage.ensure_sidecar("/a/one.jpg")
...
def test_health_and_refresh_publish_modeled_openapi_contracts(tmp_path: Path) -> None:
    ...
    refresh_schema = schema["paths"]["/refresh"]["post"]["responses"]["200"]["content"]["application/json"]["schema"]

    assert health_schema["$ref"].endswith("/HealthResponse")
    assert refresh_schema["$ref"].endswith("/RefreshResponse")
...
def test_refresh_swaps_health_index_og_and_views_from_current_context(
    ...
    monkeypatch.setattr(local_app, "ensure_preindex_storage", _fake_ensure_preindex_storage)

    app = _trusted_app(root, og_preview=True)
    with _trusted_client(app) as client:
...
def test_index_shell_is_cached_once_per_process(tmp_path: Path, monkeypatch) -> None:
    ...
    app = create_app(str(tmp_path), options=LocalAppOptions(og_preview=True))
