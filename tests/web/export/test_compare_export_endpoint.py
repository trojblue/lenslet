...
import lenslet.web.export.rendering as render_mod
import lenslet.web.export.response as response_mod
from lenslet.web.metadata import read_png_info
...
def _read_comparison_metadata(raw: bytes) -> dict | None:
    ...
    for chunk in parsed.get("found_text_chunks", []):
        if chunk.get("keyword") == response_mod.EXPORT_COMPARISON_METADATA_KEY:
            text = chunk.get("text", "")
...
def test_export_comparison_publishes_modeled_openapi_contract(tmp_path: Path) -> None:
    schema = create_app(str(tmp_path)).openapi()
    route = schema["paths"]["/export-comparison"]["post"]
    body_schema = route["requestBody"]["content"]["application/json"]["schema"]

    assert body_schema["$ref"].endswith("/ExportComparisonRequest")
    assert set(route["responses"]["200"]["content"]) >= {"image/png", "image/gif"}
    assert route["responses"]["400"]["content"]["application/json"]["schema"]["$ref"].endswith("/ErrorResponse")
    assert route["responses"]["404"]["content"]["application/json"]["schema"]["$ref"].endswith("/ErrorResponse")
...
def test_export_comparison_gif_slideshow_success(tmp_path: Path) -> None:
    ...
    assert len(response.content) <= response_mod.MAX_EXPORT_GIF_MAX_BYTES

    with Image.open(io.BytesIO(response.content)) as exported:
        assert exported.format == "GIF"
        assert bool(getattr(exported, "is_animated", False))
        assert exported.n_frames == 2
        assert max(exported.size) <= response_mod.MAX_EXPORT_GIF_LONG_SIDE
        assert exported.info.get("duration") == response_mod.EXPORT_GIF_FRAME_DURATION_MS
    ...
    assert metadata_payload["gif_max_long_side"] == response_mod.MAX_EXPORT_GIF_LONG_SIDE
    assert metadata_payload["gif_frame_duration_ms"] == response_mod.EXPORT_GIF_FRAME_DURATION_MS
...
def test_export_comparison_gif_high_quality_mode(tmp_path: Path) -> None:
    ...
    with Image.open(io.BytesIO(response.content)) as exported:
        ...
        assert bool(getattr(exported, "is_animated", False))
        assert exported.n_frames == 2
        assert max(exported.size) <= response_mod.MAX_EXPORT_GIF_LONG_SIDE_HIGH_QUALITY
        assert exported.info.get("duration") == response_mod.EXPORT_GIF_FRAME_DURATION_MS_HIGH_QUALITY

    metadata_payload = _read_gif_comparison_metadata(response.content)
    assert metadata_payload is not None
    assert metadata_payload["gif_high_quality"] is True
    assert metadata_payload["gif_max_long_side"] == response_mod.MAX_EXPORT_GIF_LONG_SIDE_HIGH_QUALITY
    assert metadata_payload["gif_frame_duration_ms"] == response_mod.EXPORT_GIF_FRAME_DURATION_MS_HIGH_QUALITY
...
def test_export_comparison_gif_enforces_size_limit(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(response_mod, "MAX_EXPORT_GIF_MAX_BYTES", 100)
    client = TestClient(create_app(str(tmp_path)))
    response = client.post(
        "/export-comparison",
        json=_export_payload(output_format="gif"),
...
def test_export_comparison_returns_422_for_undecodable_source_payload(tmp_path: Path) -> None:
    _make_png(tmp_path / "a.png")
...
def test_export_comparison_enforces_pixel_bounds(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(response_mod, "MAX_EXPORT_SOURCE_PIXELS", 200)
    ...
    monkeypatch.setattr(response_mod, "MAX_EXPORT_SOURCE_PIXELS", 64_000_000)
    monkeypatch.setattr(response_mod, "MAX_EXPORT_STITCHED_PIXELS", 500)
...
import lenslet.web.export.response
"""
    subprocess.run([sys.executable, "-c", script], check=True, capture_output=True, text=True)


def test_comparison_export_package_init_stays_narrow() -> None:
    script = """
import sys
import lenslet.web.export as export_pkg
assert "lenslet.web.export.response" not in sys.modules
assert "PIL" not in sys.modules
assert not hasattr(export_pkg, "export_comparison_response")
"""
    subprocess.run([sys.executable, "-c", script], check=True, capture_output=True, text=True)


def test_export_comparison_annotation_layout_wraps_to_source_width() -> None:
    font, lines, padding = render_mod._annotation_text_layout("x" * 120, 16)
    probe = Image.new("RGB", (1, 1))
    try:
        draw = ImageDraw.Draw(probe)
        max_width = 16 - (padding * 2)
        assert lines
        assert all(render_mod._text_size(draw, line, font)[0] <= max_width for line in lines)
    finally:
        probe.close()
