...
def test_dataset_storage_reuses_s3_client_per_instance(monkeypatch) -> None:
    ...
    first = storage.remote_access_url("s3://bucket/a.jpg")
    second = storage.remote_access_url("s3://bucket/b.jpg")
...
def test_table_storage_reuses_s3_client_per_instance(tmp_path: Path, monkeypatch) -> None:
    ...
    storage = TableStorage([{"source": str(local_image)}], options=TableStorageOptions(root=None))
    first = storage.remote_access_url("s3://bucket/c.jpg")
    second = storage.remote_access_url("s3://bucket/d.jpg")
...
def test_thumb_disconnect_cancels_queued_work_and_updates_metrics() -> None:
    ...
    try:
        ...
        class Storage:
            ...
            def get_or_build_thumbnail(self, _path: str) -> bytes:
                self.calls += 1
                return b"thumb"
...
def test_thumb_disconnect_keeps_shared_generation_for_connected_waiter() -> None:
    queue: ThumbnailScheduler[bytes | None] = ThumbnailScheduler(max_workers=1)
    try:
        started = threading.Event()
        release = threading.Event()
        ...
        async def _run_waiters(storage: Storage):
            ...
            for _ in range(100):
                if queue.stats()["waiters"] == 3:
                    break
                await asyncio.sleep(0.01)
            assert queue.stats()["waiters"] == 3
            disconnect_now.set()
            for _ in range(100):
                if queue.stats()["waiters"] == 2:
                    break
                await asyncio.sleep(0.01)
            assert queue.stats()["waiters"] == 2
            release.set()
            return await asyncio.gather(disconnected, connected)

        storage = Storage()
        disconnected_response, connected_response = asyncio.run(_run_waiters(storage))

        assert block_future.result(timeout=1.0) == b"block"
        assert disconnected_response.status_code == 204
        assert connected_response.status_code == 200
        assert connected_response.body == b"thumb"
