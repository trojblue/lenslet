...
def _build_table_app(root: Path) -> tuple[object, str, str]:
    ...
    return create_app_from_table(rows, options=TableAppOptions(base_dir=None)), "/gallery", "/gallery/a.jpg"
...
def test_table_file_route_falls_back_to_read_bytes_for_remote_sources(
    ...
    rows = [
        {
            "path": "/gallery/a.jpg",
            "source": "s3://bucket/a.jpg",
            "width": 16,
            "height": 12,
        }
    ]
    storage = TableStorage(rows, options=TableStorageOptions(root=None))
...
def test_table_mode_og_shell_path_skips_subtree_count(
    ...
    app = create_app_from_table(
        rows,
        options=TableAppOptions(base_dir=None, og_preview=True),
    )
