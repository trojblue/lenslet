...
def test_metadata_endpoint_reads_quick_view_defaults_from_fixture_qfty_meta():
    with tempfile.TemporaryDirectory() as tmp:
        root = Path(tmp)
        fixture_src = REPO_ROOT / "docs" / "test_meta.png"
...
def test_metadata_reader_rejects_text_file_like_input() -> None:
    with pytest.raises(TypeError, match="file-like input must return bytes"):
        read_png_info(io.StringIO("not bytes"))
