...
def test_recursive_first_request_uses_lightweight_recursive_loader(
    tmp_path: Path,
    monkeypatch,
) -> None:
    root = tmp_path
    ...
    def _unexpected_load_index(_path: str):
        raise AssertionError("recursive folder requests should stay on the lightweight loader path")
...
def test_recursive_count_only_avoids_snapshot_cache_work(tmp_path: Path) -> None:
    ...
    with TestClient(app) as client:
        ...
        assert count_payload["total_items"] == 8
